...
export default function Banner({ data, notice }) {
  const marqueeEl = useRef();
  const carouselEl = useRef();

  function judgeLinks(url, title) {
      ...
      <div className={styles['banner-wrapper']}>
            ...
            {data?.map((item, index) => (
              ...
              >
                <div
                  style={{
                    background: `url(${
                      (process.env.NEXT_PUBLIC_URL ?? '') + item.ImagePathH
        ...
        <div className={styles['marquee-wrapper']}>
          ...
          <div className={styles['control_box']}>
            <button onClick={() => marqueeEl.current.slickPlay()} title="播放輪播圖">
              <img src="/images/slick/slider-icon-play.png" alt={massage[`${locale}`].play} />
              <span className='d-none'>{massage[`${locale}`].play}</span>
            </button>
            <button onClick={() => marqueeEl.current.slickPause()} title="停止輪播圖">
              <img src="/images/slick/slider-icon-pause.png" alt={massage[`${locale}`].stop} />
              <span className='d-none'>{massage[`${locale}`].stop}</span>
            </button>
          </div>
        </div>
      </div>
...
function SamplePrevArrow(props) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <img src="/images/home/slider-arrow-prev.png" alt="" />
    </div>
  );
}
