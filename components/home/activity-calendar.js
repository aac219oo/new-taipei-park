...
export default function ActivityCalendar() {
  ...
  useEffect(() => {
    ...
    const lastYM = moment(new Date(time.getFullYear(), time.getMonth(), 0)).format('YYYY/MM');
    const currentYM = moment(new Date(time.getFullYear(), time.getMonth() + 1, 0)).format('YYYY/MM');
    const nextYM = moment(new Date(time.getFullYear(), time.getMonth() + 2, 0)).format('YYYY/MM');
    ...
    for (let i = firstDayIndex; i > 0; i--) {
      ...
      thisMonthArray.push({
        ...
        currentMonth: false,
        activity:
          posterData?.length > 0 ? posterData?.filter(dt =>
    ...
    <div className="row">
      <div className="col-md-12 mx-auto text-center">
        <div className={styles['control_bar']}>
          <a href="#!" onClick={() => prevMonth()} title="瀏覽上一個月的行事曆">
            <img src="/images/home/arrow-prev-white.png" alt="上一個月" />
          </a>
          <div>{controlBar}</div>
          <a href="#!" onClick={() => nextMonth()} title="瀏覽下一個月的行事曆">
                ...
                if (days.length > 35) {
                  return (
                    ...
                    >
                      ...
                      {day.activity.length === 0 ? null : (
                        <a href="#1" title="顯示當日活動(開啟懸浮視窗)" style={{ display: "block" }}>
                      ...
                      {day.activity.length ===
                        0 ? null : !showInfo ? null : showInfo.date !==
                          ...
                          <div style={{ position: 'relative' }}>
                            <div className={`${styles['detail_popup']} ${day.index % 7 < 2 ? styles['to_right'] : null}`}>
                              <div className={styles['wrapper']}>
                                <a href="#!" title="關閉當日活動訊息" onClick={() => closePopup()} className={styles['close']} style={{ display: "block" }}>
                                  <img
                                ...
                                {showInfo?.activity?.map((itm, idx) => {
                                  return (
                                    <div className={styles['item']} key={idx}>
                                      <span className={styles['tag']}>{itm.Organizer}</span>
                                      <a href={`/activity/detail?id=${itm.ID}`}>{itm.Title}</a>
                                    </div>
                                  );
                                })}
                              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {day.activity.length === 0 ? null : !showInfo ? null : showInfo.date !==
                        day.date ? null : (
